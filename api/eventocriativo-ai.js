...
Para analises com Claude, use como evidencia apenas o que estiver no historico, briefing ou resumo enviado. Se faltar algo, peca um resumo especifico para o DeepSeek em vez de pedir acesso bruto a tudo.
...
function inferTargetTitle(message) {
    ...
    const explicit = text.match(/(?:tarefa|evento|documento|doc|estrategia|estrat[Ã©e]gia)\s+(?:chamada|chamado|de nome|com titulo|com t[Ã­i]tulo)?\s*([^,.]+?)(?:\s+(?:para|de|do|da|como)\b|,|\.|$)/i);
...
function inferTitle(message, fallback) {
    ...
    const named = text.match(/(?:chamada|chamado|titulo|t[Ã­i]tulo|nomeada|nomeado)\s+(.+?)(?:\.|,|$)/i);
    if (named) return cleanTitle(named[1], fallback);

    const afterFor = text.match(/(?:tarefa|evento|reuniao|reuni[Ã£a]o|estrategia|estrat[Ã©e]gia|documento)\s+(?:para|de)?\s*(.+?)(?:\s+(?:para|no|na|dia|amanh[Ã£a]|hoje|as|Ã s)\b|\.|,|$)/i);
...
function cleanTitle(value, fallback) {
    const title = String(value || '')
        .replace(/\b(para|amanha|amanhÃ£|hoje|as|Ã s|dia)\b.*$/i, '')
        .trim();
