...
module.exports = async function handler(request, response) {
    ...
    if (process.env.PSILU_CLEANUP_SECRET) {
        const providedSecret = String(request.headers['x-psilu-cleanup-secret'] || '');
        if (providedSecret !== process.env.PSILU_CLEANUP_SECRET) {
    ...
    const cleanupResponse = await fetch(`${SUPABASE_URL}/rest/v1/psilu_ai_chats?created_at=lt.${encodeURIComponent(cutoff)}`, {
        method: 'DELETE',
        headers: {
