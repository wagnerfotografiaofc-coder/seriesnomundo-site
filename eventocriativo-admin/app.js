...
const DAILY_PHRASES = [
    'Vamos transformar eventos criativos em uma empresa escalavel.',
    'Hoje e dia de transformar arte em operacao.',
    'Menos barulho, mais decisao.',
    'A Evento Criativo cresce quando o foco vira rotina.',
    'Um evento bem organizado vira conteudo, venda e aprendizado.',
    'A estrategia aparece quando os dados da empresa param de ficar soltos.'
];

const CHAT_STORAGE_PREFIX = 'eventocriativo-active-chat';
...
const tables = {
    tasks: 'eventocriativo_tasks',
    events: 'eventocriativo_calendar_events',
    editorial: 'eventocriativo_editorial_items',
    docs: 'eventocriativo_docs',
    strategies: 'eventocriativo_strategies',
    chats: 'eventocriativo_ai_chats',
    messages: 'eventocriativo_ai_messages',
    actions: 'eventocriativo_ai_suggested_actions'
};
...
const modalConfigs = {
    ...
    doc: {
        ...
        fields: [
            ['title', 'Titulo', 'text', true],
            ['category', 'Categoria', 'select:ICP|Ofertas|Eventos|Concorrentes|Instagram|Produto|Vendas|Analises|Financeiro|Operacao|Outro', true],
...
async function sendChatMessage(event) {
    ...
    try {
        ...
        const response = await fetch('/api/eventocriativo-ai', {
            method: 'POST',
            headers: {
